...
impl DecisionTree {
              ...
              G: Fn(T, &mut [usize], usize, f32) -> (&mut [usize], &mut [usize])
    {
        ...
        for _ in 0..10 {
            ...
            for (feature_position, &feature_idx) in candidate_features.iter().enumerate() {
                ...
                let (threshold, loss) = DecisionTree::calculate_impurity_split(feature_values);
    ...
    fn calculate_impurity_split(values: &FeatureValues) -> (f32, f32) {
    ...
    fn calculate_variance_split(values: &FeatureValues) -> (f32, f32) {
        let (_, max_value) = values.value_bounds();

        let mut split_variance = f32::INFINITY;
        let mut split_x = 0.0;
...
mod tests {
    ...
    fn calculate_impurity_split_1() {
        ...
        let (threshold, impurity) = DecisionTree::calculate_impurity_split(&values);
